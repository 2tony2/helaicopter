...
def test_collect_orchestration_facts_emits_prefect_run_and_attempt_facts(tmp_path: Path) -> None:
    ...
    (flow_run_dir / "metadata.json").write_text(
        """
        {
          ...
          "run_title": "Run: Prefect Analytics",
    ...
    run_facts, task_attempt_facts = collect_orchestration_facts(tmp_path)
    run_fact = next(fact for fact in run_facts if fact.run_source == "prefect_local")
    task_fact = next(fact for fact in task_attempt_facts if fact.run_source == "prefect_local")

    assert run_fact.status == "completed"
    assert run_fact.task_attempt_count == 1
    assert task_fact.task_id == "plan"
    assert task_fact.model == "gpt-5"
