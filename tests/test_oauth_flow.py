...
def test_initiate_oauth_returns_redirect_url() -> None:
    original = auth_application._OAUTH_CLIENTS.copy()
    auth_application._OAUTH_CLIENTS["claude"] = _StubOAuthClient()
    try:
...
def test_oauth_callback_stores_credential() -> None:
    ...
    try:
        with _oauth_client() as client:
            initiate = client.post("/auth/credentials/oauth/initiate", json={"provider": "claude"})
