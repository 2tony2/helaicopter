...
- Auth management wiring: `/orchestration#auth-management` shows `Reuse Claude CLI session` for Claude and `OAuth redirect` for Codex.
...
- Local auth remains provider-specific during manual smoke verification:
  ...
  - Codex requires `HELA_CODEX_OAUTH_CLIENT_ID` plus a working local OAuth callback configuration.
...
- Confirm Codex offers `OAuth redirect`.
- Click Claude connect and verify a credential appears when the local Claude CLI is already authenticated.
- Click Codex OAuth and verify the redirect begins without a `Failed to fetch` error.
