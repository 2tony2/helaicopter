...
Filesystem access, SQLite access, and artifact reads stay in `adapters/` behind interfaces defined by `ports/`. Neither domain nor request-layer code should know external layout details directly.
...
python/helaicopter_api/
  ...
  adapters/      # filesystem/sqlite/artifact implementations
