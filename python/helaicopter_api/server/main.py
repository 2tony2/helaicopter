...
def create_app() -> FastAPI:
    ...
    mcp_mount_holder: dict[str, Any] = {}

    @asynccontextmanager
    async def combined_lifespan(application: FastAPI) -> AsyncIterator[None]:
        async with lifespan(application):
            # FastMCP's HTTP app owns a one-shot session manager, so we must
            # rebuild and reattach it on each parent app startup.
            mcp_app = application.state.mcp_server.http_app(path="/")
            mcp_app_holder["app"] = mcp_app
            mcp_mount_holder["route"].app = mcp_app
    ...
    mcp_mount_holder["route"] = application.routes[-1]
    return application
